...
![Policy Draft to Publication workflow](./diagrams/policy-draft-to-publication.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Policy Draft | `policy_draft` |
...
- `policy_draft` → process step candidate
...
- `publication` → process step candidate
...
bp:policydrafttopublication a ng:BusinessProcessPattern ;
  ng:processId "BP-141" ;
  ng:domain "Governance Risk Data" ;
  rdfs:label "Policy Draft to Publication" .
```
