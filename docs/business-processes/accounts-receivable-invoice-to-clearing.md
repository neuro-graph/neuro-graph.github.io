---
title: Accounts Receivable Invoice to Clearing
category: Finance & Controlling
processId: BP-076
---

# Accounts Receivable Invoice to Clearing
...
![Accounts Receivable Invoice to Clearing workflow](./diagrams/accounts-receivable-invoice-to-clearing.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Accounts Receivable Invoice | `accounts_receivable_invoice` |
...
- `accounts_receivable_invoice` → process step candidate
...
- `clearing` → process step candidate
...
bp:accountsreceivableinvoicetoclearing a ng:BusinessProcessPattern ;
  ng:processId "BP-076" ;
  ng:domain "Finance & Controlling" ;
  rdfs:label "Accounts Receivable Invoice to Clearing" .
```
