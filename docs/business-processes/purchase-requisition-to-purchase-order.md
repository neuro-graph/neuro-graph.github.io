...
![Purchase Requisition to Purchase Order workflow](./diagrams/purchase-requisition-to-purchase-order.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Purchase Requisition | `purchase_requisition` |
...
- `purchase_requisition` → process step candidate
...
- `purchase_order` → process step candidate
...
bp:purchaserequisitiontopurchaseorder a ng:BusinessProcessPattern ;
  ng:processId "BP-023" ;
  ng:domain "Procurement & Supplier" ;
  rdfs:label "Purchase Requisition to Purchase Order" .
```
