---
title: Accrual Calculation to Reversal
category: Finance & Controlling
processId: BP-074
---

# Accrual Calculation to Reversal
...
![Accrual Calculation to Reversal workflow](./diagrams/accrual-calculation-to-reversal.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Accrual Calculation | `accrual_calculation` |
...
- `accrual_calculation` → process step candidate
...
- `reversal` → process step candidate
...
bp:accrualcalculationtoreversal a ng:BusinessProcessPattern ;
  ng:processId "BP-074" ;
  ng:domain "Finance & Controlling" ;
  rdfs:label "Accrual Calculation to Reversal" .
```
