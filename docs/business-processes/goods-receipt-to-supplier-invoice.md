---
title: Goods Receipt to Supplier Invoice
...
![Goods Receipt to Supplier Invoice workflow](./diagrams/goods-receipt-to-supplier-invoice.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Goods Receipt | `goods_receipt` |
...
| 6 | Supplier Invoice | `supplier_invoice` |
...
- `goods_receipt` → process step candidate
...
- `supplier_invoice` → process step candidate
...
bp:goodsreceipttosupplierinvoice a ng:BusinessProcessPattern ;
  ng:processId "BP-026" ;
  ng:domain "Procurement & Supplier" ;
  rdfs:label "Goods Receipt to Supplier Invoice" .
```
