---
title: Service Entry to Invoice
category: Procurement & Supplier
processId: BP-035
---

# Service Entry to Invoice
...
![Service Entry to Invoice workflow](./diagrams/service-entry-to-invoice.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Service Entry | `service_entry` |
...
- `service_entry` → process step candidate
...
- `invoice` → process step candidate
...
bp:serviceentrytoinvoice a ng:BusinessProcessPattern ;
  ng:processId "BP-035" ;
  ng:domain "Procurement & Supplier" ;
  rdfs:label "Service Entry to Invoice" .
```
