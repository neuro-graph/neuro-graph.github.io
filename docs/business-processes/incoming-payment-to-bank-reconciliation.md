---
title: Incoming Payment to Bank Reconciliation
category: Finance & Controlling
processId: BP-077
---

# Incoming Payment to Bank Reconciliation
...
![Incoming Payment to Bank Reconciliation workflow](./diagrams/incoming-payment-to-bank-reconciliation.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Incoming Payment | `incoming_payment` |
...
| 6 | Bank Reconciliation | `bank_reconciliation` |
...
- `incoming_payment` → process step candidate
...
- `bank_reconciliation` → process step candidate
...
bp:incomingpaymenttobankreconciliation a ng:BusinessProcessPattern ;
  ng:processId "BP-077" ;
  ng:domain "Finance & Controlling" ;
  rdfs:label "Incoming Payment to Bank Reconciliation" .
```
