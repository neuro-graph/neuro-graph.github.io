---
title: Service Request to Closure
...
![Service Request to Closure workflow](./diagrams/service-request-to-closure.svg)

## Primary workflow

| Step | Workflow stage | Suggested RDF role |
|---:|---|---|
| 1 | Service Request | `service_request` |
...
- `service_request` → process step candidate
...
- `closure` → process step candidate
...
bp:servicerequesttoclosure a ng:BusinessProcessPattern ;
  ng:processId "BP-012" ;
  ng:domain "Customer & Revenue" ;
  rdfs:label "Service Request to Closure" .
```
